...
export default function Characters() {
  ...
  useEffect(() => {
    getCharacters(IP_ID).then((d) => {
      setList(d.data)
      if (d.data[0]) setSel(d.data[0].id)
    })
    getRelations(IP_ID).then(setGraph)
  }, [])
  ...
  const trendOption = useMemo(() => ({
    ...
    yAxis: [
      { type: 'value', axisLabel: { color: muted, fontSize: 10 }, splitLine: { lineStyle: { color: 'rgba(200,155,60,0.08)' } } },
      { type: 'value', axisLabel: { color: muted, fontSize: 10 }, splitLine: { show: false } },
    ],
    series: [
      { name: '讨论量', type: 'line', smooth: true, symbol: 'none', data: trend.map((t) => t.discussions), lineStyle: { color: gold }, areaStyle: { color: 'rgba(200,155,60,0.12)' } },
    ...
    return {
      ...
      series: [{
        ...
        links: graph.edges.map((e) => ({
          ...
          lineStyle: { color: 'rgba(200,155,60,0.45)', curveness: 0.15 },
        })),
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '40px 32px 64px' }}>
      ...
      <div className="xj-panel" style={{ marginBottom: 20 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '0.6fr 1fr 1fr 1fr 1fr 1fr 0.8fr', padding: '8px 18px', borderBottom: '1px solid rgba(200,155,60,0.12)', fontSize: '0.625rem', color: '#6B6258' }}>
        ...
        {list.map((c, i) => (
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              border: 'none',
              borderBottom: i < list.length - 1 ? '1px solid rgba(200,155,60,0.04)' : 'none',
              background: sel === c.id ? 'rgba(200,155,60,0.08)' : 'transparent',
              color: ink,
              fontFamily: 'inherit',
            }}
          >
