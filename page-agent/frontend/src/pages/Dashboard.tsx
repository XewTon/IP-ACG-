...
function MetricBox({label,value,change}:{label:string;value:number;change:string}){
  return (
    <div className="xj-panel" style={{padding:'16px 18px'}}>
      ...
      <div style={{fontSize:'1.25rem',fontWeight:700,color:'var(--xj-ink)',fontFamily:'"Noto Serif SC",serif'}}>{typeof value==='number'&&value<100?value+'%':value.toLocaleString()}</div>
      <div style={{fontSize:'0.625rem',color:change.startsWith('+')?'#6a8a6a':'var(--xj-gold)',marginTop:2}}>{change}</div>
    </div>
  )
}
...
export default function Dashboard() {
  ...
  if(!metrics)return <div style={{padding:'48px 32px',color:'var(--xj-faint)'}}>加载中...</div>
    ...
    <div style={{maxWidth:960,margin:'0 auto',padding:'48px 32px'}}>
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between'}}>
        <div>
          ...
          <p style={{fontSize:'0.625rem',color:'var(--xj-muted)',margin:'0 0 28px'}}>四维指标：内容 · 用户 · 供应链 · IP健康</p>
        </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:24}}>
        ...
        <div className="xj-panel" style={{padding:'16px 18px'}}>
          ...
          {Object.entries(metrics.ipHealth.characterHeat as Record<string,number>).map(([k,v])=>(
            <div key={k} style={{display:'flex',justifyContent:'space-between',fontSize:'0.6875rem',color:'var(--xj-faint)',marginTop:2}}><span>{k}</span><span style={{color:'var(--xj-red)'}}>{v}</span></div>
          ))}
        </div>
        <div className="xj-panel" style={{padding:'16px 18px'}}>
          <div style={{fontSize:'0.625rem',color:'var(--xj-muted)',marginBottom:4}}>内容生命周期</div>
          <div style={{fontSize:'1.25rem',fontWeight:700,color:'var(--xj-ink)',fontFamily:'"Noto Serif SC",serif'}}>{metrics.ipHealth.contentLifecycle.value}天</div>
        </div>
      </div>
