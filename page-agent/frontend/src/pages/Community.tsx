...
const levelStyle:Record<string,{dot:string;bg:string}> = {green:{dot:'#6a8a6a',bg:'rgba(106,138,106,0.06)'},yellow:{dot:'#C89B3C',bg:'rgba(200,155,60,0.06)'},red:{dot:'#9b2d30',bg:'rgba(155,45,48,0.06)'}}
...
export default function Community() {
  ...
  const inputStyle:React.CSSProperties = { background:'#0a0c14',border:'1px solid rgba(200,155,60,0.15)',color:'#e8e0d0',padding:'7px 10px',fontSize:'0.75rem',fontFamily:'"Noto Sans SC",sans-serif' }
  ...
  return (
    <div style={{maxWidth:900,margin:'0 auto',padding:'48px 32px'}}>
      ...
      {/* 玩家画像 */}
      <h3 style={{fontSize:'0.75rem',color:'#C89B3C',marginBottom:12,fontFamily:'"Noto Serif SC",serif'}}>玩家画像</h3>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:28}}>
        {personas.map((p)=>(
          <div key={p.id} className="xj-panel" style={{padding:16}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
              <span style={{fontSize:'0.8125rem',fontWeight:500,color:'#e8e0d0'}}>{p.type}</span>
            ...
            <div style={{fontSize:'0.625rem',color:'#6B6258',borderTop:'1px solid rgba(200,155,60,0.08)',paddingTop:6}}>→ {p.action}</div>
            <button onClick={()=>savePersona(p)} style={{background:'none',border:'none',color:'#C89B3C',cursor:'pointer',fontSize:'0.5625rem',marginTop:6}}>保存</button>
          </div>
        ))}
      </div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
        <h3 style={{fontSize:'0.75rem',color:'#C89B3C',margin:0,fontFamily:'"Noto Serif SC",serif'}}>用户反馈池</h3>
        <button onClick={()=>setShowFb(true)} className="xj-btn" style={{padding:'5px 14px',fontSize:'0.625rem'}}>+ 添加反馈</button>
      </div>
      <div style={{display:'flex',gap:8,marginBottom:12}}>
        {['','positive','neutral','negative'].map(s=>(
          <button key={s} onClick={()=>setSentFilter(s)} style={{background:sentFilter===s?'rgba(200,155,60,0.12)':'transparent',color:sentFilter===s?'#C89B3C':'#8a8578',border:'none',fontSize:'0.625rem',cursor:'pointer',padding:'3px 10px',fontFamily:'"Noto Sans SC",sans-serif'}}>{s?{positive:'正面',neutral:'中立',negative:'负面'}[s]:'全部'}</button>
        ))}
      </div>
      <div className="xj-panel" style={{marginBottom:28}}>
        <div style={{display:'grid',gridTemplateColumns:'50px 60px 1fr 50px 30px',padding:'8px 18px',borderBottom:'1px solid rgba(200,155,60,0.12)',fontSize:'0.625rem',color:'#6B6258'}}><span>平台</span><span>用户</span><span>反馈</span><span>情感</span><span/></div>
        {feedbacks.filter(f=>!sentFilter||f.sentiment===sentFilter).map((f)=>(
          <div key={f.id} className="xj-row" style={{padding:'0 18px'}}>
            ...
            <span style={{fontSize:'0.75rem',color:'#e8e0d0',width:60,flexShrink:0}}>{f.user_name}</span>
      ...
      {/* 社区事件 */}
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
        <h3 style={{fontSize:'0.75rem',color:'#C89B3C',margin:0,fontFamily:'"Noto Serif SC",serif'}}>社区事件日志</h3>
      ...
      <div className="xj-panel">
        {events.map((e)=>(
          <div key={e.id} style={{display:'flex',alignItems:'center',padding:'12px 20px',gap:14,borderBottom:'1px solid rgba(200,155,60,0.06)',background:levelStyle[e.level].bg}}>
            ...
            <span style={{fontSize:'0.75rem',color:'#e8e0d0',flex:1}}>{e.title}</span>
      ...
      {/* 反馈 Modal */}
      {showFb&&(
        <div style={{position:'fixed',inset:0,background:'rgba(4,4,8,0.85)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100}} onClick={()=>setShowFb(false)}>
          <div className="xj-panel" style={{padding:'28px 32px',maxWidth:420,width:'90%'}} onClick={e=>e.stopPropagation()}>
            <h3 style={{fontSize:'0.9375rem',color:'#C89B3C',margin:'0 0 16px',fontFamily:'"Noto Serif SC",serif'}}>添加反馈</h3>
            ...
            <div style={{display:'flex',gap:10,marginTop:16,justifyContent:'flex-end'}}>
              <button onClick={()=>setShowFb(false)} style={{background:'transparent',color:'#8a8578',border:'1px solid rgba(200,155,60,0.2)',padding:'8px 20px',fontSize:'0.75rem',cursor:'pointer'}}>取消</button>
      ...
      {/* 事件 Modal */}
      {showEv&&(
        <div style={{position:'fixed',inset:0,background:'rgba(4,4,8,0.85)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100}} onClick={()=>setShowEv(false)}>
          <div className="xj-panel" style={{padding:'28px 32px',maxWidth:420,width:'90%'}} onClick={e=>e.stopPropagation()}>
            <h3 style={{fontSize:'0.9375rem',color:'#C89B3C',margin:'0 0 16px',fontFamily:'"Noto Serif SC",serif'}}>添加事件</h3>
            <div style={{display:'flex',flexDirection:'column',gap:10}}>
            ...
            <div style={{display:'flex',gap:10,marginTop:16,justifyContent:'flex-end'}}>
              <button onClick={()=>setShowEv(false)} style={{background:'transparent',color:'#8a8578',border:'1px solid rgba(200,155,60,0.2)',padding:'8px 20px',fontSize:'0.75rem',cursor:'pointer'}}>取消</button>
