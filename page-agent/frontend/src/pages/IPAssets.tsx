  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '48px 32px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <h3 style={{ fontSize: '0.75rem', color: '#C89B3C', margin: 0, fontFamily: '"Noto Serif SC",serif' }}>角色资产库</h3>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {characters.map((c, i) => (
          <button key={c.id} onClick={() => setSelChar(i)} style={{ background: selChar === i ? 'rgba(200,155,60,0.12)' : 'transparent', color: selChar === i ? '#C89B3C' : '#8a8578', border: '1px solid rgba(200,155,60,0.15)', padding: '4px 16px', fontSize: '0.75rem', cursor: 'pointer', fontFamily: '"Noto Sans SC",sans-serif' }}>{c.name}</button>
        ))}
      </div>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 6 }}>
                <span style={{ fontSize: '1rem', fontWeight: 700, color: '#e8e0d0', fontFamily: '"Noto Serif SC",serif' }}>{ch.name}</span>
                <span style={{ fontSize: '0.6875rem', color: '#C89B3C' }}>{ch.role}</span>
                <span style={{ fontSize: '0.625rem', color: '#6B6258', border: '1px solid rgba(200,155,60,0.15)', padding: '1px 8px' }}>{ch.tag}</span>
                <button onClick={() => openEdit(ch)} style={{ background:'none', border:'none', color:'#C89B3C', cursor:'pointer', fontSize:'0.625rem', marginLeft:'auto' }}>编辑</button>
              ...
              <div style={{ borderTop: '1px solid rgba(200,155,60,0.1)', paddingTop: 14 }}>
                <div style={{ fontSize: '0.6875rem', color: '#C89B3C', marginBottom: 8 }}>版本历史</div>
                {ch.versions.map((v, i) => (
                  <div key={i} style={{ display: 'flex', gap: 12, padding: '6px 0', fontSize: '0.6875rem', borderBottom: i < ch.versions.length - 1 ? '1px solid rgba(200,155,60,0.04)' : 'none' }}>
                    <span style={{ color: '#C89B3C', width: 110, flexShrink: 0 }}>{v.version}</span>
      ...
      <h3 style={{ fontSize: '0.75rem', color: '#C89B3C', marginBottom: 12, fontFamily: '"Noto Serif SC",serif' }}>世界观时间线</h3>
      <div className="xj-panel" style={{ padding: 16, marginBottom: 28 }}>
        {data.lore.map((t, i) => (
          <div key={i} style={{ display: 'flex', gap: 14, padding: '10px 8px', borderBottom: i < data.lore.length - 1 ? '1px solid rgba(200,155,60,0.06)' : 'none' }}>
            <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#C89B3C', width: 60, flexShrink: 0, fontFamily: '"Noto Serif SC",serif' }}>{t.date_label}</span>
            <span style={{ fontSize: '0.75rem', color: '#8a8578' }}>{t.event}</span>
          </div>
        ))}
      </div>

      <h3 style={{ fontSize: '0.75rem', color: '#C89B3C', marginBottom: 12, fontFamily: '"Noto Serif SC",serif' }}>IP 规范中心</h3>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
        {data.rules.map((r, i) => (
          <div key={i} className="xj-panel" style={{ padding: 18 }}>
            <div style={{ fontSize: '0.75rem', fontWeight: 500, color: '#C89B3C', marginBottom: 10 }}>{r.category}</div>
      ...
      {/* 添加/编辑角色 Modal */}
      {showAdd && (
        <div style={{ position:'fixed',inset:0,background:'rgba(4,4,8,0.85)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100 }} onClick={()=>setShowAdd(false)}>
          <div className="xj-panel" style={{ padding:'28px 32px',maxWidth:420,width:'90%' }} onClick={e=>e.stopPropagation()}>
            <h3 style={{ fontSize:'0.9375rem',color:'#C89B3C',margin:'0 0 16px',fontFamily:'"Noto Serif SC",serif' }}>{editId ? '编辑角色' : '添加角色'}</h3>
            <div style={{ display:'flex',flexDirection:'column',gap:10 }}>
              {[{k:'name',l:'名称'},{k:'role',l:'身份定位'},{k:'tag',l:'标签'},{k:'keywords',l:'关键词'},{k:'description',l:'描述'},{k:'assets',l:'已有素材'}].map(({k,l})=>(
                <input key={k} placeholder={l} value={(form as any)[k]} onChange={e=>setForm({...form,[k]:e.target.value})}
                  style={{ background:'#0a0c14',border:'1px solid rgba(200,155,60,0.15)',color:'#e8e0d0',padding:'8px 12px',fontSize:'0.8125rem',fontFamily:'"Noto Sans SC",sans-serif' }} />
              ))}
            </div>
            <div style={{ display:'flex',gap:10,marginTop:16,justifyContent:'flex-end' }}>
              <button onClick={()=>setShowAdd(false)} style={{ background:'transparent',color:'#8a8578',border:'1px solid rgba(200,155,60,0.2)',padding:'8px 20px',fontSize:'0.75rem',cursor:'pointer' }}>取消</button>
